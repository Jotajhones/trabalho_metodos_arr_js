...
export function findIndex(valorDigitado, todosOsDados) {
    const result = document.querySelector('.result');
    const termoBusca = valorDigitado.toLowerCase().trim();

    // Lógica: Retorna a posição (índice) usando uma regra (função) ao invés de um valor exato
    const posicao = todosOsDados.findIndex(item => item.id === termoBusca);

    if (posicao !== -1) {
        const metodoEncontrado = todosOsDados[posicao];

        result.innerHTML = `
        <div style="color: #15803d; margin-bottom: 10px;"><strong>✔ Encontrado via FINDINDEX! (Posição ${posicao})</strong></div>
        <div><span class="tagMainContent">Nome: </span>${metodoEncontrado.nome}</div>
        <div><span class="tagMainContent">Tipo: </span>${metodoEncontrado.tipo}</div>
        `;
    } else {
        result.innerHTML = `<div style="color: #b91c1c;"><strong>✖ Falso (-1):</strong> Nenhum objeto atende a condição.</div>`;
    }
}

export function findLast(valorDigitado, todosOsDados) {
    const result = document.querySelector('.result');
    const termoBusca = valorDigitado.toLowerCase().trim();

    // Lógica: Faz a mesma coisa que o Find, mas varre o array de trás pra frente
    const metodoEncontrado = todosOsDados.findLast(item => item.id === termoBusca);

    if (metodoEncontrado) {
        result.innerHTML = `
        <div style="color: #15803d; margin-bottom: 10px;"><strong>✔ Encontrado via FINDLAST! (Busca Reversa)</strong></div>
        <div><span class="tagMainContent">Nome: </span>${metodoEncontrado.nome}</div>
        <div><span class="tagMainContent">Tipo: </span>${metodoEncontrado.tipo}</div>
        `;
    } else {
...
export function findLastIndex(valorDigitado, todosOsDados) {
    const result = document.querySelector('.result');
    const termoBusca = valorDigitado.toLowerCase().trim();

    // Lógica: Pega a posição usando uma regra (função), mas começando do fim do array
    const posicao = todosOsDados.findLastIndex(item => item.id === termoBusca);

    if (posicao !== -1) {
        const metodoEncontrado = todosOsDados[posicao];

        result.innerHTML = `
        <div style="color: #15803d; margin-bottom: 10px;"><strong>✔ Encontrado via FINDLASTINDEX! (Posição ${posicao})</strong></div>
        <div><span class="tagMainContent">Nome: </span>${metodoEncontrado.nome}</div>
        <div><span class="tagMainContent">Tipo: </span>${metodoEncontrado.tipo}</div>
        `;
    } else {
